var PrettyFormatter = function(options) {
  ...
  self.handleBeforeScenarioOutlineEvent = function handleBeforeScenarioOutlineEvent(event, callback) {
	var originalGetPayloadItem = event.getPayloadItem;
	event.getPayloadItem = function(name){
		return originalGetPayloadItem.call(event, 'scenarioOutline');
	}
	self.handleBeforeScenarioEvent(event, callback);
  };

  self.handleAfterScenarioOutlineEvent =  function handleAfterScenarioOutlineEvent(event, callback) {
	var originalGetPayloadItem = event.getPayloadItem;
	event.getPayloadItem = function(name){
		return originalGetPayloadItem.call(event, 'scenarioOutline');
	}
	self.handleAfterScenarioEvent(event, callback);
  };
