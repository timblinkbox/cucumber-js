var AstTreeWalker = function(features, supportCodeLibrary, listeners) {
  ...
  if(typeof stepStart === 'function'){
    var originalProcessStep = self.processStep;
    self.processStep = function processStep(step, callback) {
      if (self.isStepUndefined(step)) {
        ...
        stepStart(step.getKeyword() + ': ' + step.getName(), function () { throw ('Error: Step definition could not be found.\n\nExpected a step definition similar to the following:\n\n' + snippet); });
      }

      originalProcessStep.call(self, step, callback);
    };
  }
